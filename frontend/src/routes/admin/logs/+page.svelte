  ...
  let filteredLogs = $derived(
    logs
      ...
      .sort((a, b) => {
        ...
        const aActor = (a.actorName || '').toLowerCase();
        const bActor = (b.actorName || '').toLowerCase();
        const aDetail = (a.actionDetail || '').toLowerCase();
        const bDetail = (b.actionDetail || '').toLowerCase();

        // Check Actor Name
        const aActorExact = aActor === query;
        const bActorExact = bActor === query;
        if (aActorExact && !bActorExact) return -1;
        if (!aActorExact && bActorExact) return 1;

        const aActorStarts = aActor.startsWith(query);
        const bActorStarts = bActor.startsWith(query);
        if (aActorStarts && !bActorStarts) return -1;
        if (!aActorStarts && bActorStarts) return 1;

        // Check Action Detail
        const aDetailExact = aDetail === query;
        const bDetailExact = bDetail === query;
        if (aDetailExact && !bDetailExact) return -1;
        if (!aDetailExact && bDetailExact) return 1;

        const aDetailStarts = aDetail.startsWith(query);
        const bDetailStarts = bDetail.startsWith(query);
        if (aDetailStarts && !bDetailStarts) return -1;
        if (!aDetailStarts && bDetailStarts) return 1;
