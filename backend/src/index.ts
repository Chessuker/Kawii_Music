...
app.get('/api/tracks', async (c) => {
  try {
    ...
    const paginatedTracks = await db.select().from(tracks)
      ...
      .orderBy(
        // 1: Exact Match
        desc(drizzleSql`CASE WHEN ${tracks.title} ILIKE ${searchTitle} THEN 1 ELSE 0 END`),
        
        // 2: Start With
        desc(drizzleSql`CASE WHEN ${tracks.title} ILIKE ${searchTitle + '%'} THEN 1 ELSE 0 END`),
        
        // 3: Most Views
        desc(tracks.viewCount),
...
app.get('/api/albums/grouped', async (c) => {
  try {
    ...
    const artistList = await db.select()
        ...
        .where(artistCond)
        .orderBy(
          // อันดับ 1: ถ้าชื่อตรงเป๊ะ 100% (Exact Match) เอาขึ้นบนสุด
          desc(drizzleSql`CASE WHEN ${artists.name} ILIKE ${search} THEN 1 ELSE 0 END`),
          
          // อันดับ 2: ถ้าชื่อขึ้นต้นด้วยคำนี้ (Start With) เอาไว้รองลงมา
          desc(drizzleSql`CASE WHEN ${artists.name} ILIKE ${search + '%'} THEN 1 ELSE 0 END`),

          // สุดท้าย: ถ้าคะแนนเท่ากันหมด ค่อยเรียงตามตัวอักษร A-Z
          artists.name
        )
        .limit(limit)
        .offset(offset);
...
app.get('/api/merch', async (c) => {
  try {
    ...
    const allItems = await db.select().from(items).where(conditions).orderBy(
      // อันดับ 1: ถ้าชื่อตรงเป๊ะ 100% (Exact Match) เอาขึ้นบนสุด
      desc(drizzleSql`CASE WHEN ${items.name} ILIKE ${search} THEN 1 ELSE 0 END`),
      
      // อันดับ 2: ถ้าชื่อขึ้นต้นด้วยคำนี้ (Start With) เอาไว้รองลงมา
      desc(drizzleSql`CASE WHEN ${items.name} ILIKE ${search + '%'} THEN 1 ELSE 0 END`),

      // สุดท้าย: ถ้าคะแนนเท่ากันหมด ค่อยเรียงจากใหม่ไปเก่า
      desc(items.id)
    );
...
app.get('/api/admin/reports/active-users', async (c) => {
  try {
    ...
    const activeUsers = await db.select({
      ...
      // Weight: ฟัง 1 เพลง = 1 แต้ม, สร้าง 1 เพลย์ลิสต์ = 5 แต้ม
...
app.get('/api/admin/reports/high-value-purchases', async (c) => {
  try {
    ...
    .where(
        and(
            // เอาบิลที่ราคารวม > ผลลัพธ์จาก avgQuery
